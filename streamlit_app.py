...
if not os.path.exists(os.path.join(BASE_DIR, "bornes.db")):
    # Base absente : (re)génération depuis les sources publiques OpenData Ville de Paris.
...
def charger_donnees():
    ...
    if os.path.exists(db_path):
        ...
        # population fournie par build_data (OpenData) ; pas de collecte externe ici
...
with st.sidebar:
    ...
    if st.button("🔄 Rafraîchir les données", width='stretch'):
            with st.spinner("Mise à jour depuis l'OpenData Ville de Paris…"):
                try:
                    import build_data
                    build_data.main()
                    st.cache_data.clear()
                    st.success("✅ Données rafraîchies depuis l'OpenData public")
                except Exception as e:
                    st.warning(f"Rafraîchissement indisponible : {e}")
            st.rerun()
...
with tab_population:
    if population is None or population.empty:
        ...
        st.markdown("### Population par arrondissement")
        st.markdown(
        "> Population totale de chaque arrondissement parisien — utilisée pour rapporter "
        "l'équipement en bornes à la densité de population (source INSEE)."
            )
        ...
        df_display = df_pop[["label", "pop_total"]].copy()
        df_display.columns = ["Arrondissement", "Pop. totale"]
        df_display = df_display.sort_values("Pop. totale", ascending=False).reset_index(drop=True)

        st.dataframe(
            df_display.style.format({
                "Pop. totale": "{:,.0f}",
            }, na_rep="—").background_gradient(subset=["Pop. totale"], cmap="Greens"),
            width='stretch',
            hide_index=True,
        )
